...
class Associations
{
    ...
    /**
     * This is just a list of `name => type` of the
    ...
    public function associations()
    {
        if (!$this->associations) {
            if ($this->getService('rails.config')['use_cache']) {
                $assocs = $this->getCachedData();
            } else {
                $assocs = $this->getAssociationsData();
            }
            $this->embedded = $assocs['embedded'];
            unset($assocs['embedded']);
    ...
    public function isEmbedded($name)
    {
        return isset($this->embedded[$name]);
    }
    
    /**
     * Get embedded associated objects.
     *
     * @return array
     */
    public function embedded()
    {
        ...
        // $assocs = [];
        // foreach ($this->embedded as $name => $type) {
            // $assocs[$type] = $this->load($name);
        // }
        // return $assocs;
    }
